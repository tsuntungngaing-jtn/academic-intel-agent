...
从项目根目录（``academic-intel-agent``）::

    python src/interactive_agent.py
...
from processor.analyzer import iter_jsonl
...
def parse_line(line: str) -> tuple[Optional[str], Optional[int], bool]:
    """
    Returns ``(action, index, cite)`` where action is ``'quit'`` | ``'help'`` | ``'lookup'`` | None.
    """
    s = line.strip()
    if not s:
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    report_path = args.report or (_project_data_dir() / "final_report.jsonl")
